...
function numbersOperations() {
  if (numbers.length > 0) {
    ...
    if (numbers.length === 1 && typeof numbers[0] === "number") {
      console.log(
        `La raíz cuadrada de ${numbers[0]} es: ${
          Number.isInteger(Math.sqrt(numbers[0]))
            ? Math.sqrt(numbers[0])
      ...
      for (i = 1; i < numbers.length; i++) {
        resultSume += numbers[i];
        resultRest -= numbers[i];
        resultMult *= numbers[i];
        resultDiv /= numbers[i];
      }
      console.log(
        `El resultado de la suma es : ${
          Number.isInteger(resultSume) ? resultSume : resultSume.toFixed(3)
        }.`
      );
      console.log(
...
function moreOperations() {
  const moreOps = prompt("Quiere realizar mas operaciones ? si / no");
  if (moreOps === "si") {
    resetArray(numbers);
    calculatorPro();
  } else {
