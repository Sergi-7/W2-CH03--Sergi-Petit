...
function flightsList() {
    for (let i = 0; i < flights.length; i++) {
        ...
        console.log(`El vuelo ID ${flights[i].id} con origen ${flights[i].from} y destino ${flights[i].to} tiene un coste de ${flights[i].cost}€ y no realiza escala.`)
    }
...
function getAveragePrice() {
  let totalPrice = 0;
    ...
    console.log(`El precio medio de los vuelos es de ${Math.round(totalPrice/flights.length)} euros.`);
};
...
function getScaleFlights() {
    ...
    console.log(`Hay un total de ${scaleFlights.length} vuelos que realizan escala.`);
};
...
function adminOrUser() {
    let adminOrUser = prompt('Eres administrador o usuario ? Escriba: admin / user')
...
function removeFlight() {
    let idFlight = Math.round(parseInt(prompt('Introduzca el ID del vuelo que desea eliminar :')));
...
function priceSearcher() {
    ...
    let userOrder = prompt('Desea buscar vuelos por encima de un precio, por debajo, o iguales a un precio en concreto ? Mas caros / Mas Baratos / Igual a').toLowerCase();
    ...
    if (userOrder === 'mas caros') {
        ...
        for (i= 0; i < flights.length; i++) {
            ...
            console.log(`El vuelo con un precio mas cercano a ${price} es el ID ${flights[i].id} con origen ${flights[i].from} y destino ${flights[i].to} y tiene un precio de ${flights[i].cost} €`)
            }
        }
    }
};
...
function getMoreExpensiveThan(num) {
    for (i=0; i < flights.length; i++) {
        if (flights[i].cost >= num) {
            console.log(`El vuelo con id ${flights[i].id} con origen ${flights[i].from} y destino ${flights[i].to} tiene un precio de ${flights[i].cost} y ${flights[i].scale ? 'si' : 'no'} realiza escala. `)
        }
    }
  };
...
function buyFlight() {
    let buy = prompt('Si quiere comprar el vuelo introduzca su ID, en caso contrario haga click en cancelar :');
    let index;
    for(i = 0; i < flights.length; i++) {
    ...
    if ( buy !== null) {
    console.log(`Ha elegido el vuelo ID ${flights[index].id} con origen ${flights[index].from} y destino ${flights[index].to} y un precio de ${flights[index].cost} €. Gracias por su compra !`)
    } else  {
